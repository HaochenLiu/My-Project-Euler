...
int main() {
    ...
    bignum d = lltobignum(1);
    contFrac.push_back(2);
    for(int i = 1; i < N; i++) {
        contFrac.push_back(1);
        contFrac.push_back(2 * i);
        contFrac.push_back(1);
    }
    contFrac.push_back(1);
    contFrac.push_back(2 * N);

    reverse(contFrac.begin(), contFrac.end());

    cout<<"sz = "<<contFrac.size()<<endl;
    for(int i = 0; i < contFrac.size(); i++) {
        swap(n, d);
        n = n + (contFrac[i] * d);
    }

    cout<<"n "<<bignumtos(n)<<" d "<<bignumtos(d)<<endl;
    cout<<"Result "<<getDigitSum(n)<<endl;
